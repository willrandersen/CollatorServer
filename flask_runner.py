...
def show_past_search(task_id):
    ...
    response_html = template.format(task_id, search_object.name, search_object.user_name, (search_object.search_started - time_delt).strftime("%b %d %Y %H:%M:%S") + " CDT </td>", get_bolded_dict_string(search_object.items_searched),task_id,table_html_string)
